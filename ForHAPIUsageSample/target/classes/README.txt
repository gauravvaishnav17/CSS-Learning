...
The purpose of NDHM FHIR Sample Codes is to demonstrate how to populate, validate clinical artifacts against NDHM profiles, serialize and parse NDHM clinical artifacts programmatically using FHIR HAPI library that can be captured and exchanged as per NDHM Health Data Interchange Specifications 1.0. 
...
2. Dependencies required
   ...
   e. hapi-fhir-structures-r5-5.0.2.jar (As of HAPI-FHIR v0.8, a separate FHIR structure JAR must be added to your classpath (or project pom.xml if you are using Maven))
...
1. Sample Codes
	...
	b. PrescriptionSample
		Populate Prescription sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

    c. OPConsultNoteSample
		Populate OPConsultNote sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

	d. DischargeSummarySample
		Populate DischargeSummary sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

	e. DiagnosticReportLabSample
		Populate DiagnosticReportLab sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

	f. DiagnosticReportImagingMediaSample
		Populate DiagnosticReportImagingMedia sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

	g. DiagnosticReportImagingDcmSample
		Populate DiagnosticReportImagingDcm sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.

	h. ImmunizationRecordSample
		Populate ImmunizationRecord sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.
	
	i. WellnessRecordSample
		Populate WellnessRecord sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.
		
	j. HealthDocumentRecordSample
		Populate HealthRecord sample bundle, validates populated bundle, serialize it and write it in json/xml format. Then parse the existing written file and validates it again.
